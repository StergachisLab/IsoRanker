...
# -----------------------------
# NEW helper: parse GTF gene_type
# -----------------------------
def load_protein_coding_gene_names_from_gtf(gtf_path: str) -> set:
    """
    Returns a set of gene_name values (symbols) whose gene_type is 'protein_coding'
    based on attributes in a (possibly gzipped) GTF.

    Assumes your long_format_annotated['associated_gene'] contains gene symbols
    like gene_name in the GTF (e.g., "DDX11L16").
    """
    ...
    with opener(gtf_path, "rt") as fh:
        for line in fh:
            ...
            # Fast substring check to skip most lines
            if 'gene_type "protein_coding"' not in attrs:
                continue

            # Extract gene_name "XYZ";
            # (attribute order can vary, so search)
            key = 'gene_name "'
            i = attrs.find(key)
            if i == -1:
                continue
            ...
            gene_name = attrs[i:j].strip()
            if gene_name:
                pc_genes.add(gene_name)
...
def main():
    ...
    expression_matrix = create_expression_matrix(
        read_stat_path,
        output_file=os.path.join(output_dir, "expression_matrix.tsv.gz")
    )
    ...
    ########################
    # NEW: Filter to protein_coding genes per GTF
    ########################
    print("Loading protein_coding genes from GTF", flush=True)
    protein_coding_genes = load_protein_coding_gene_names_from_gtf(gtf_path_input)

    before_pc = len(long_format_annotated)
    long_format_annotated = long_format_annotated[
        long_format_annotated["associated_gene"].isin(protein_coding_genes)
    ].copy()
    after_pc = len(long_format_annotated)

    print(
        f"Filtered to protein_coding genes (by GTF gene_type): {before_pc:,} -> {after_pc:,} "
        f"(protein_coding gene_name count in GTF: {len(protein_coding_genes):,})",
        flush=True
    )

    long_format_annotated.to_csv(
        os.path.join(output_dir, "long_format_annotated.tsv.gz"),
        index=False,
        compression="gzip",
        sep="\t"
    )
    ...
    separated_results_files = {
        ...
        "Cyclo_Allelic_Imbalance_isoform_top_ranked_data.tsv.gz",
        "Noncyclo_Allelic_Imbalance_isoform_top_ranked_data.tsv.gz"
