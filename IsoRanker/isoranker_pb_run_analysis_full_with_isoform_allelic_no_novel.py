...
def main():
    ...
    ######################## Only filter for non-novel genes ########################
    ...
    # Remove genes that contain "novelGene" anywhere in the name (case-sensitive by default)
    mask = ~long_format_annotated["associated_gene"].str.contains("novelGene", na=False)

    # Optional: also drop empty/"nan"
    mask &= long_format_annotated["associated_gene"].notna()
    mask &= long_format_annotated["associated_gene"].str.lower().ne("nan")
    mask &= long_format_annotated["associated_gene"].ne("")

    long_format_annotated = long_format_annotated[mask].copy()
    after_n = len(long_format_annotated)

    print(f"Filtered to non-novel genes (removed 'novelGene'): {before_n:,} -> {after_n:,}", flush=True)
